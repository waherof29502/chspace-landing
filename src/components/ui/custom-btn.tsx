...
export default function CustomBtn({title,fontType}:ButtonProps) {
  return (
     <button className='flex mx-12 ml-[125px] xs:ml-[150px] md:ml-0 my-[10px] md:my-0 py-[50px] transition-all '>
        ...
        <p className={`text-primary translate-x-13 ${fontType} text-[16px] tracking-[1.48px] transition-all  group-hover:translate-x-12 ease-linear duration-800`}>{title}</p>
        <div className="relative w-[60px] group-hover:w-5 h-[1.2px] bg-primary transform transition-transform group-hover:ease-linear group-hover:translate-x-6 duration-800" />
