...
export default function Service() {
  ...
  return (
    ...
    <div className={`flex flex-row justify-center items-center font-serif mt-5 md:mt-10 `}>
        ...
        <div className="hidden lg:flex lg:pl-10 xl:pl-12 3xl:pl-0 xl:justify-center pt-[80px]">
            ...
            <div className='flex flex-row md:justify-start items-center px-0 xl:px-8 wide:px-10 xl:pt-4'>
              <h2 className='font-syne text-[30px] md:text-[48px] text-primary font-normal capitalize'>
               Service
              </h2>
              <span className='pl-4 mt-1.5 font-sansCjk text-[#545351] text-[16px] xl:text-[18px]'>
                服務項目
              </span>
            </div>
            <div className='flex flex-row items-center xl:pl-3 pt-5 py-[20px] xl:py-[50px] 3xl:py-[80px]'>
              {SERVICESFILTER.map((item)=>(
                ...
                <div className='text-[30px] xl:text-[40px] text-[#D9D9D9] group-hover:text-[#1B1B1B] pb-4'>{item.icon}</div>
            ...
            <span className="font-sansCjk leading-[42px] text-[#1B1B1B] tracking-[1.76px] font-medium text-[14px] xxxl:text-[16px] xl:pl-[20px] lg:pr-10 xl:pr-1 wide:pr-[120px]">我們專注於高端商業的綜合服務，包括商業空間升級以打造城市文化地標、文化空間企劃與設計、文化內容策劃等，透過淬鏈、創意、多元的設計概念及操作手法，以最純粹的方式，表現品牌價值及故事。</span>
    ...
    <div className="lg:hidden flex flex-col w-full justify-center">
             <div className='flex flex-row md:justify-start items-center px-10 pt-4 pb-10'>
              <h2 className='font-syne text-[30px] md:text-[48px] text-primary font-normal capitalize'>
               Service
              </h2>
            ...
            <span className="font-sansCjk leading-[42px] text-[#1B1B1B] tracking-[1.76px] font-medium text-[16px] pt-[80px] px-[40px]">我們專注於高端商業的綜合服務，包括商業空間升級以打造城市文化地標、文化空間企劃與設計、文化內容策劃等，透過淬鏈、創意、多元的設計概念及操作手法，以最純粹的方式，表現品牌價值及故事。</span>
    ...
    <div className='grid grid-cols-1 md:grid-cols-2 gap-[20px] xl:gap-[200px] '>
         <ul className='md:hidden '>
        {PROCESSINFO.map((item, index) => (
            <li className='relative flex gap-4 mt-[40px] items-center' key={item.id}>
            <div className='flex justify-center items-center bg-[#F3F3F3] rounded-full w-[80px] h-[80px] z-10'>
        ...
        {PROCESSINFO.slice(0,4).map((item, index) => (
            <li className='relative flex gap-4 xxxl:gap-8 md:pt-[95px] lg:pt-[85px] 3xl:pt-[70px] items-center' key={item.id}>
    ...
    <div className='relative bg-cover bg-center w-full h-[531px] pb-10' style={{ backgroundImage: `url(${NewsBg.src})` }}>
        <div className='flex flex-col md:justify-center lg:pl-[70px] h-full md:w-[60%] 2md:w-1/2 px-10 md:px-0 md:ml-auto '>
        ...
        <div className='hidden md:flex flex-col w-2/3'>
            <span className='text-primary text-[16px] tracking-[1.76px] py-2'>我們將為您提供適合您需求的「最佳成本建議」。</span>
            <span className='text-primary text-[16px] tracking-[1.76px] py-2'>我們相信僅靠價格並不能反映我們服務的價值和滿意度。</span>
            <span className='text-primary text-[16px] tracking-[1.76px] py-2'>在聽取您的詢問後，我們將為您提供設計及施工建議等成本的估算。</span>
        </div>
            <span className='md:hidden w-full text-primary text-[16px] leading-[35px] tracking-[2.08px] mx-auto'>我們將為您提供適合您需求的「最佳成本建議」。 我們相信僅靠價格並不能反映我們服務的價值和滿意度。 在聽取您的詢問後，我們將為您提供設計及施工建議等成本的估算。</span>
        </div>
    </div>    
    </section>
  )
